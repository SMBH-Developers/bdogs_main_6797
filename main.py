...
async def send_text_with_name(message: types.Message, scheduler: SchedulerSingleton):
    ...
    self_message = await client.send_message(message.from_user.id, text=f'{name + ", п" if name else "П"}одскажите, пожалуйста, Вы ранее проходили диагностику своих чакр?')
